...
There is no secret-store call here. OCI Vault cannot survive an apply/destroy
loop — a deleted vault occupies the tenancy's single vault slot for 30 days —
so the password is generated by Terraform and read back out of state. See
`03-openclaw/accounts.tf`.
